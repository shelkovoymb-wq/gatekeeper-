...
## Вход в кабинет: 2FA и Google/Яндекс

Кабинет поддерживает три способа входа, которые сочетаются между собой.

| Метод | Эндпоинт (API) | Через BFF (кабинет) |
|---|---|---|
| Пароль | `POST /v1/auth/login` | `POST /api/auth/login` |
| Второй фактор | `POST /v1/auth/2fa/verify` | `POST /api/auth/2fa/verify` |
| Google / Яндекс | `GET /v1/auth/oauth/:p/url` → `POST /v1/auth/oauth/:p/callback` | `GET /api/auth/oauth/:p/start` |

**Двухфакторная аутентификация (TOTP).** Реализация своя, на `node:crypto`
(RFC 4226/6238, SHA-1 / 6 цифр / 30 с) — совместима с Google Authenticator,
Яндекс.Ключом, 1Password. Подключение в `Настройки → Двухфакторная защита`:
QR → код из приложения → 10 одноразовых резервных кодов (показываются один раз).

- Секрет лежит зашифрованным (`SecretBox`, AES-256-GCM) — дамп БД без
  `SECRET_ENCRYPTION_KEY` кодов не даёт.
- Если 2FA включена, `login` **не** выдаёт сессию: возвращается
  короткоживущий challenge-токен (5 мин), который BFF кладёт в httpOnly-cookie
  и обменивает на сессию после ввода кода.
- Один и тот же код не проходит дважды (запоминаем номер 30-секундного шага),
  резервный код сгорает при использовании, после 5 неверных попыток —
  временная блокировка на 15 минут поверх общего rate limit по IP.

**Вход через Google и Яндекс.** Authorization Code Flow; секреты провайдеров
не покидают API, редирект-часть обрабатывает BFF, поэтому токен сессии сразу
попадает в httpOnly-cookie и не светится в адресной строке.

- `redirect_uri` вычисляется на бэкенде из `OAUTH_REDIRECT_BASE_URL`
  (по умолчанию `PUBLIC_APP_URL`), а не берётся из запроса — открытого
  редиректа нет by design.
- CSRF: `state` подписан JWT (провайдер + режим + одноразовый nonce, 10 мин)
  **и** дополнительно сверяется с httpOnly-cookie в BFF.
- Первый вход склеивается с существующим аккаунтом только по **подтверждённой
  провайдером** почте; иначе создаётся новый (или, при
  `OAUTH_ALLOW_SIGNUP=false`, вход отклоняется).
- Включённая 2FA обязательна и при входе через провайдера.
- Привязка/отвязка — в `Настройки → Вход через сервисы`. Последний способ
  входа отвязать нельзя: у OAuth-аккаунта пароля нет, пока он его не задаст.

Ключи провайдеров и параметры — в `.env.example` (секции «ДВУХФАКТОРНАЯ
АУТЕНТИФИКАЦИЯ» и «ВХОД ЧЕРЕЗ GOOGLE / ЯНДЕКС»). Без ключей соответствующая
кнопка просто не показывается, вход по паролю работает как раньше.

### Тесты

```bash
pnpm test                    # юнит-тесты (БД не нужна)
# интеграционные тесты аутентификации на живой БД:
pnpm --filter @gatekeeper/api db:migrate
TEST_DATABASE_URL=postgres://gatekeeper:gatekeeper@localhost:5432/gatekeeper pnpm test
```
