...
| `ADDON_PAYMENT_SECRET` | Секрет кабинета шлюза для подписи вебхука платных опций. **Без него `/billing/addon/webhook` отвечает 503 и ничего не активирует** |
| `MIN_ADDON_AMOUNT`    | Порог активации платной опции (по умолчанию 100) |
| `UPLOADS_DIR`         | Каталог вложений постов (по умолчанию `/data/uploads`, том `uploads`) |
...
| Эндпоинт есть в коде, но снаружи 404 | Наружу через nginx проброшены только `/healthz`, `/tg/`, `/payments/webhook/` и `/billing/addon/` — остальное уходит в Next.js. Это защита, а не баг | Владельческие и клиентские ручки вызываются из браузера через BFF `/api/*`, который ходит в API по внутренней сети. Новый эндпоинт для кабинета → префикс `v1/...` + BFF-роут в `apps/web/src/app/api/` |
| Сборка `web` падает с `ERR_PNPM_PNPM_ENGINE_IDENTITY_UNVERIFIABLE` («Cannot verify the identity of the @pnpm/exe.linux-x64 native binary») | `apps/web/Dockerfile` ставил pnpm без версии, и однажды `npm install -g pnpm` притащил свежий, а он проверяет идентичность пакетного менеджера и падает на нашем lock-файле. Образ `api` берёт версию через `corepack` из поля `packageManager` и потому не пострадал (27.08.2026) | Версия в Dockerfile закреплена (`pnpm@9.12.0`) и должна совпадать с `packageManager` в корневом `package.json`. При смене одной — менять и вторую |
| Конфиг nginx на .44 разошёлся с репозиторием | Файл `/opt/gatekeeper-proxy/nginx.conf` правится на хосте руками и в git не возвращается. 27.08.2026 на проде обнаружилась версия ДО security-fix: наружу был проброшен весь `/v1/`, то есть сервис-токен-эндпоинты были доступны из интернета | Сверять `md5sum` живого файла с `deploy/pve3/gatekeeper-proxy/nginx.conf` при каждой выкатке, затрагивающей маршруты. Обновление: `cp nginx.conf nginx.conf.bak.$(date +%s)`, положить версию из репозитория, `docker exec gatekeeper-proxy-gatekeeper-proxy-1 nginx -t`, затем `nginx -s reload` |
| Загрузка вложения поста отваливается на `413` | Общий `client_max_body_size 4m` в nginx на .44, а Telegram принимает файлы до 50 МБ | Для пути `/api/posts/media` заведён отдельный `location` с лимитом 50 МБ (см. `deploy/pve3/gatekeeper-proxy/nginx.conf`). После правки конфига — `docker exec gatekeeper-proxy nginx -s reload` на .44 |
| Клиент оплатил опцию, доступа нет | Разбирается по журналу событий шлюза, а не по логам | `/owner/addons` → «Журнал шлюза»: `invalid_signature` — проверить `ADDON_PAYMENT_SECRET`; `client_not_found` — платёж без `order_num`; `rejected_low_amount` — сумма ниже `MIN_ADDON_AMOUNT`; событий нет вовсе — вебхук до нас не дошёл, проверить адрес уведомлений в кабинете шлюза и `location /billing/addon/` |
| Запланированный пост не вышел | Задача живёт в Redis, а срок — в базе; после чистки Redis задача пропадает | Крон добирает просроченные `scheduled` раз в пять минут — достаточно подождать. Если пост в статусе `failed`, причина в самом посте: `posts.error` и `post_targets.error` (обычно бот потерял права в канале) |
